...
def totals_tree(m):
    ...
    if is_planet(m):
        return tree(size(m))
    mbranch =  [totals_tree(end(t)) for t in m[1:]]
    return tree(total_weight(m),mbranch)
...
def preorder(t):
    ...
    if is_leaf(t):
        return [label(t)] 
    return reduce(add,[preorder(b) for b in branches(t)], [label(t)])
...
def has_path(t, phrase):
    ...
    def rec(t,phrase,root= False ):   
        if root == True and label(t) != phrase[0]:
            return False
        if len(phrase) == 1 and label(t) == phrase[0]:
            ...
            res = False
            for b in branches(t):
                res = res or rec(b,phrase[1:])
            return res
        else : 
            res = False
            for b in branches(t):
                res = res or rec(b,phrase)
            return res
    return rec(t,phrase,True)
...
def sub_interval(x, y):
    ...
    and any value in y."""
    newy = interval(-upper_bound(y),-lower_bound(y))
    return add_interval(x,newy)
