...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while p != beehive:        
            if len(p.bees) >= 1 :
                if distance >= self.min_range and distance <= self.max_range and distance != self.ex_dis:    
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if len(self.place.bees) >= 1:
            ...
            self.digesting = (self.digesting + 1) % (self.time_to_digest +1)
...
class WallAnt(Ant):
    name = 'Wall'
    food_cost =4
    # OVRERIDE CLASS ATTRIBUTES 
    implemented = True # whether can be seen in GUI
...
# END Problem 13
        ...
        if self.first == 0: 
            ...
            while p != None:
                if p.ant is not None:
                    if not p.ant.inspired:
                        p.ant.damage *= 2
                        p.ant.inspired = True
                    if isinstance(p.ant,ContainerAnt) and p.ant.contained_ant is not None:
                        if not p.ant.contained_ant.inspired: 
    ...
    def reduce_armor(self, amount):
        ...
        if self.first == 0 and self.armor <= 0 :
            bees_win()
        # END Problem 13
